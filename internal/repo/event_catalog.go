...
func (db *DB) ListEventCatalogEntries(ctx context.Context, clusterID string) ([]domain.EventCatalogDoc, error) {
	rows, err := db.pool.Query(ctx, queryListEventCatalogEntries, clusterID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) GetEventCatalogEntry(ctx context.Context, clusterID, subject string) (EventCatalogEntryRow, error) {
	row := db.pool.QueryRow(ctx, queryGetEventCatalogEntry, clusterID, subject)
...
func (db *DB) UpsertEventCatalogEntry(
	ctx context.Context,
	...
	_, err = db.pool.Exec(ctx, queryUpsertEventCatalogEntry,
	...
	return db.GetEventCatalogEntry(ctx, clusterID, canonical)
}

func (db *DB) DeleteEventCatalogEntry(ctx context.Context, clusterID, subject string) error {
	canonical, err := domain.CanonicalEventCatalogSubject(subject)
	if err != nil {
		return err
	}
	tag, err := db.pool.Exec(ctx, queryDeleteEventCatalogEntry, clusterID, canonical)
	if err != nil {
		return err
	}

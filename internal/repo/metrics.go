...
func (db *DB) InsertMetricSamples(ctx context.Context, clusterID string, capturedAt time.Time, samples []MetricSampleRow) error {
	if len(samples) == 0 {
		return nil
	}
	capturedAt = capturedAt.UTC().Truncate(time.Second)
	if err := db.ensureDayPartition(ctx, metricSamplesParent, capturedAt); err != nil {
		return err
	}
...
func (db *DB) DeleteMetricSamplesOlderThan(ctx context.Context, cutoff time.Time) (int64, error) {
	return db.dropParentPartitionsOlderThan(ctx, metricSamplesParent, cutoff.UTC())
}

func (db *DB) QueryMetricSeries(
	ctx context.Context,
	clusterID string,
	metrics []string,
	from, to time.Time,

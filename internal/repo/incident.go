...
func (db *DB) InsertIncidentAnnotation(ctx context.Context, clusterID string, in domain.IncidentAnnotationCreate) (domain.IncidentAnnotation, error) {
	...
	_, err := db.pool.Exec(ctx, queryInsertIncidentAnnotation,
...
func (db *DB) ListIncidentAnnotations(ctx context.Context, clusterID string, from, to time.Time) ([]domain.IncidentAnnotation, error) {
	rows, err := db.pool.Query(ctx, queryListIncidentAnnotations, clusterID, from.UTC(), to.UTC())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) InsertIncidentConsumerSamples(ctx context.Context, clusterID string, capturedAt time.Time, samples []domain.IncidentConsumerSample) error {
	if len(samples) == 0 {
		return nil
	}
	capturedAt = capturedAt.UTC().Truncate(time.Second)
	if err := db.ensureDayPartition(ctx, incidentSamplesParent, capturedAt); err != nil {
		return err
	}
...
func (db *DB) ListIncidentConsumerSamples(
	ctx context.Context,
	clusterID, stream, consumer string,
	from, to time.Time,
) ([]domain.IncidentConsumerSample, error) {
	rows, err := db.pool.Query(ctx, queryListIncidentConsumerSamples,
...
func (db *DB) InsertIncidentNodeEvents(ctx context.Context, clusterID string, events []domain.IncidentNodeEvent) error {
	if len(events) == 0 {
		return nil
	}
	var b strings.Builder
...
func (db *DB) ListIncidentNodeEvents(ctx context.Context, clusterID string, from, to time.Time) ([]domain.IncidentNodeEvent, error) {
	rows, err := db.pool.Query(ctx, queryListIncidentNodeEvents, clusterID, from.UTC(), to.UTC())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) ListAuditInRange(ctx context.Context, clusterID string, from, to time.Time, limit int) ([]domain.IncidentAuditChange, error) {
	if limit <= 0 {
		limit = 200
	}
	if limit > 500 {
	...
	rows, err := db.pool.Query(ctx, queryListAuditInRange, clusterID, from.UTC(), to.UTC(), limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) DeleteIncidentDataOlderThan(ctx context.Context, cutoff time.Time) (int64, error) {
	cutoff = cutoff.UTC()
	total, err := db.dropParentPartitionsOlderThan(ctx, incidentSamplesParent, cutoff)
	if err != nil {
		return total, err
	}
	for _, q := range []string{
		...
		tag, err := db.pool.Exec(ctx, q, cutoff)
		if err != nil {
			return total, err
		}

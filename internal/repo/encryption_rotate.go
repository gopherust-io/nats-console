...
func (db *DB) RotateEncryptionKeys(ctx context.Context, currentKey, newKey string, dryRun bool) (EncryptionRotationStats, error) {
	...
	for clusterRows.Next() {
		...
		plain, err := decryptWithFallback(token, oldEnc, db.encryptor)
		if err != nil {

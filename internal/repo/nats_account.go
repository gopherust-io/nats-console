...
func (db *DB) ListNATSAccountUsers(ctx context.Context, clusterID, accountName string) ([]NATSAccountUser, error) {
	rows, err := db.pool.Query(ctx, queryListNATSAccountUsers, clusterID, accountName)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) CreateNATSAccountUser(ctx context.Context, in NATSAccountUserCreate) (NATSAccountUserCreds, error) {
	return db.CreateNATSAccountUserWithSeed(ctx, in, "")
}

func (db *DB) CreateNATSAccountUserWithSeed(ctx context.Context, in NATSAccountUserCreate, accountSeed string) (NATSAccountUserCreds, error) {
	...
	if err := db.EnsureDefaultSigningGroup(ctx, in.ClusterID, in.AccountName); err != nil {
	...
	encSeed, err := db.encryptToken(strings.BytesToString(seed))
	if err != nil {
	...
	if !strings.IsEmpty(accountSeed) {
		userJWT, err = mintUserJWT(ctx, db, in.ClusterID, in.AccountName, user, strings.BytesToString(seed), accountSeed)
		if err != nil {
			return NATSAccountUserCreds{}, err
		}
	}
	...
	id := newID()
	now := time.Now().UTC()
	_, err = db.pool.Exec(ctx, queryInsertNATSAccountUser,
...
func (db *DB) DeleteNATSAccountUser(ctx context.Context, clusterID, accountName, userID string) ([]string, error) {
...
func (db *DB) GetNATSAccountUser(ctx context.Context, clusterID, accountName, userID string) (NATSAccountUser, error) {
	item, _, err := scanNATSAccountUser(db.pool.QueryRow(ctx, queryGetNATSAccountUser, clusterID, accountName, userID).Scan)
...
func (db *DB) GetNATSAccountUserCreds(ctx context.Context, clusterID, accountName, userID string) (NATSAccountUserCreds, error) {
	var item NATSAccountUser
	var encSeed, jwtStr string
	var timeRangesJSON []byte
	err := db.pool.QueryRow(ctx, queryGetNATSAccountUserCreds, clusterID, accountName, userID).
		Scan(
	...
	item.HasJWT = !strings.IsEmpty(jwtStr)
	item.JWTIssued = !strings.IsEmpty(jwtStr)
	seed, err := db.decryptToken(encSeed)
	if err != nil {
		return NATSAccountUserCreds{}, err
	}
...
func (db *DB) UpdateNATSAccountUser(ctx context.Context, clusterID, accountName, userID string, in NATSAccountUserUpdate, accountSeed string) (NATSAccountUser, error) {
	if strings.IsEmpty(in.SigningGroup) {
		in.SigningGroup = "Default"
	}
	normalizeUserUpdateLimits(&in)
	existing, err := db.GetNATSAccountUserCreds(ctx, clusterID, accountName, userID)
	if err != nil {
		return NATSAccountUser{}, err
	}
	now := time.Now().UTC()
	_, err = db.pool.Exec(ctx, queryUpdateNATSAccountUser,
	...
	updated, err := db.GetNATSAccountUser(ctx, clusterID, accountName, userID)
	if err != nil {
		return NATSAccountUser{}, err
	}
	if !strings.IsEmpty(accountSeed) {
		userJWT, mintErr := mintUserJWT(ctx, db, clusterID, accountName, updated, existing.Seed, accountSeed)
		if mintErr != nil {
			return NATSAccountUser{}, mintErr
		}
		_, err = db.pool.Exec(ctx, queryUpdateNATSAccountUserJWT,
...
func (db *DB) RotateNATSAccountUser(ctx context.Context, clusterID, accountName, userID, accountSeed string) (NATSAccountUserCreds, error) {
	existing, err := db.GetNATSAccountUser(ctx, clusterID, accountName, userID)
	if err != nil {
		return NATSAccountUserCreds{}, err
	}
	kp, err := nkeys.CreateUser()
	...
	userJWT := ""
	if !strings.IsEmpty(accountSeed) {
		userJWT, err = mintUserJWT(ctx, db, clusterID, accountName, existing, strings.BytesToString(seed), accountSeed)
		if err != nil {
			return NATSAccountUserCreds{}, err
		}
	}
	now := time.Now().UTC()
	_, err = db.pool.Exec(ctx, queryRotateNATSAccountUser,
		clusterID, accountName, userID, pub, encSeed, userJWT, now)
	if err != nil {
		return NATSAccountUserCreds{}, err
	}
	existing.PublicKey = pub
	existing.HasJWT = !strings.IsEmpty(userJWT)
	existing.JWTIssued = !strings.IsEmpty(userJWT)
	existing.UpdatedAt = now
	seedStr := strings.BytesToString(seed)
	return NATSAccountUserCreds{
		NATSAccountUser: existing,
		Seed:            seedStr,
		JWT:             userJWT,
		Cred:            formatCreds(userJWT, seedStr),
	}, nil
}

func (db *DB) MintNATSAccountUserJWT(ctx context.Context, clusterID, accountName, userID, accountSeed string) (NATSAccountUserCreds, error) {
	if strings.IsEmpty(accountSeed) {
		return NATSAccountUserCreds{}, errors.New("NATS_ACCOUNT_SEED is not configured")
	}
	creds, err := db.GetNATSAccountUserCreds(ctx, clusterID, accountName, userID)
	if err != nil {
		return NATSAccountUserCreds{}, err
	}
	userJWT, err := mintUserJWT(ctx, db, clusterID, accountName, creds.NATSAccountUser, creds.Seed, accountSeed)
	if err != nil {
		return NATSAccountUserCreds{}, err
	}
	now := time.Now().UTC()
...
func (db *DB) AssignNATSAccountUserPerson(ctx context.Context, clusterID, accountName, natsUserID, consoleUserID string) (NATSAccountUser, error) {
	current, err := db.GetNATSAccountUser(ctx, clusterID, accountName, natsUserID)
	if err != nil {
		return NATSAccountUser{}, err
	}
	...
	return db.GetNATSAccountUser(ctx, clusterID, accountName, natsUserID)
}
...
func (db *DB) ListNATSAccountExports(ctx context.Context, clusterID, accountName, kind string) ([]NATSAccountExport, error) {
	rows, err := db.pool.Query(ctx, queryListNATSAccountExports, clusterID, accountName, kind)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) CreateNATSAccountExport(ctx context.Context, in NATSAccountExportCreate) (NATSAccountExport, error) {
	if strings.IsEmpty(in.AccountName) {
		in.AccountName = "Default"
	}
	id := newID()
	now := time.Now().UTC()
	var item NATSAccountExport
	err := db.pool.QueryRow(ctx, queryInsertNATSAccountExport,
...
func (db *DB) UpdateNATSAccountExport(ctx context.Context, clusterID, accountName, exportID string, in NATSAccountExportUpdate) (NATSAccountExport, error) {
	now := time.Now().UTC()
	var item NATSAccountExport
	err := db.pool.QueryRow(ctx, queryUpdateNATSAccountExport,
...
func (db *DB) DeleteNATSAccountExport(ctx context.Context, clusterID, accountName, exportID string) error {
	tag, err := db.pool.Exec(ctx, queryDeleteNATSAccountExport, clusterID, accountName, exportID)
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {

...
func (db *DB) InsertAudit(ctx context.Context, in AuditCreate) error {
	details, err := serializer.Marshal(in.Details)
	if err != nil {
		details = commonstrings.StringToBytes("{}")
	}
	_, err = db.pool.Exec(ctx, queryInsertAudit,
...
func (db *DB) ListAudit(ctx context.Context, f AuditFilter) ([]AuditEntry, int, error) {
	...
	var total int
	if err := db.pool.QueryRow(ctx, queryCountAudit+where, args...).Scan(&total); err != nil {
		return nil, 0, err
	}
...
func (db *DB) DeleteAuditOlderThan(ctx context.Context, cutoff time.Time) (int64, error) {
	tag, err := db.pool.Exec(ctx, queryDeleteAuditOlderThan, cutoff.UTC())
	if err != nil {
		return 0, err
	}

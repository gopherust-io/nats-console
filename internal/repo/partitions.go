...
var partitionIdentRe = regexp.MustCompile(`^[a-z][a-z0-9_]*$`)

func validPartitionIdent(name string) bool {
	if !partitionIdentRe.MatchString(name) {
		return false
	}
	return strings.HasPrefix(name, metricSamplesParent+"_") ||
		strings.HasPrefix(name, incidentSamplesParent+"_")
}
...
func (db *DB) EnsureSamplePartitions(ctx context.Context, now time.Time, retention time.Duration) error {
	now = now.UTC()
	...
	for d := start; !d.After(end); d = d.Add(24 * time.Hour) {
		if err := db.ensureDayPartition(ctx, metricSamplesParent, d); err != nil {
			return err
		}
		if err := db.ensureDayPartition(ctx, incidentSamplesParent, d); err != nil {
			return err
		}
	}
	return nil
...
func (db *DB) ensureDayPartition(ctx context.Context, parent string, day time.Time) error {
	day = day.UTC().Truncate(24 * time.Hour)
	key := parent + "|" + day.Format("2006-01-02")
	if _, ok := db.ensuredPartitions.Load(key); ok {
		return nil
	}
	name := fmt.Sprintf("%s_%s", parent, day.Format("2006_01_02"))
	if !validPartitionIdent(name) {
	...
	db.ensuredPartitions.Store(key, struct{}{})
	return nil
}
...
func (db *DB) DropSamplePartitionsOlderThan(ctx context.Context, cutoff time.Time) (int64, error) {
	cutoff = cutoff.UTC()
	var dropped int64
	for _, parent := range []string{metricSamplesParent, incidentSamplesParent} {
		n, err := db.dropParentPartitionsOlderThan(ctx, parent, cutoff)
		if err != nil {
			return dropped, err
		}
		dropped += n
...
func (db *DB) dropParentPartitionsOlderThan(ctx context.Context, parent string, cutoff time.Time) (int64, error) {
	...
	for rows.Next() {
		...
		if !upper.After(cutoff) {
			if !validPartitionIdent(name) {
				return dropped, fmt.Errorf("refusing to drop invalid partition name %q", name)
			}
			if _, err := db.pool.Exec(ctx, "DROP TABLE IF EXISTS "+quoteIdent(name)); err != nil {
				return dropped, err
			}
			dropped++
		}

...
func TestStreamConfigRequestToNATSSynadiaFields(t *testing.T) {
	...
	cfg, err := (streamConfigRequest{
		...
		Sources: []streamSourceRequest{
			{Name: "ORIGIN", FilterSubject: "orders.*"},
		},
		SubjectTransform: &subjectTransformRequest{Source: "in.>", Destination: "out.>"},
		RePublish:        &rePublishRequest{Destination: "audit.>", HeadersOnly: true},
		ConsumerLimits: &consumerLimitsRequest{
...
func TestStreamConfigRequestMirrorClearsSubjects(t *testing.T) {
	...
	cfg, err := (streamConfigRequest{
		...
		Mirror:   &streamSourceRequest{Name: "ORIGIN", FilterSubject: "x.*"},
	}).toNATS()
	require.NoError(t, err)

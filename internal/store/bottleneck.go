...
func (s *Store) UpsertBottleneckHourBuckets(ctx context.Context, rows []domain.BottleneckHourBucket) error {
	if len(rows) == 0 {
		return nil
	}
...
func (s *Store) ListBottleneckHourBuckets(
	ctx context.Context,
	clusterID string,
	from, to time.Time,
) ([]domain.BottleneckHourBucket, error) {
	rows, err := s.pool.Query(ctx, queryListBottleneckHourRollups, clusterID, from.UTC(), to.UTC())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) DeleteBottleneckHourBucketsOlderThan(ctx context.Context, cutoff time.Time) (int64, error) {
	tag, err := s.pool.Exec(ctx, queryDeleteBottleneckHourRollupsOlderThan, cutoff.UTC())
	if err != nil {
		return 0, err
	}

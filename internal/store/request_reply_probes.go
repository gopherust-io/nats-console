...
func (s *Store) ListRequestReplyProbes(ctx context.Context, clusterID string) ([]domain.RequestReplyProbe, error) {
	rows, err := s.pool.Query(ctx, queryListRequestReplyProbes, clusterID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) ListRequestReplyProbesPublic(ctx context.Context, clusterID string) ([]domain.RequestReplyProbe, error) {
	probes, err := s.ListRequestReplyProbes(ctx, clusterID)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ListEnabledRequestReplyProbes(ctx context.Context, clusterID string) ([]domain.RequestReplyProbe, error) {
	rows, err := s.pool.Query(ctx, queryListEnabledRequestReplyProbes, clusterID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) GetRequestReplyProbe(ctx context.Context, clusterID, probeID string) (domain.RequestReplyProbe, error) {
	row := s.pool.QueryRow(ctx, queryGetRequestReplyProbe, clusterID, probeID)
...
func (s *Store) CreateRequestReplyProbe(ctx context.Context, clusterID string, in domain.RequestReplyProbeCreate) (domain.RequestReplyProbe, error) {
	...
	existing, err := s.ListRequestReplyProbes(ctx, clusterID)
	if err != nil {
		return domain.RequestReplyProbe{}, err
	}
	...
	_, err = s.pool.Exec(ctx, queryInsertRequestReplyProbe,
		id, clusterID, subject, payload, format.String(), timeoutMs, enabled,
	)
	if err != nil {
		return domain.RequestReplyProbe{}, err
	}
	return s.GetRequestReplyProbe(ctx, clusterID, id)
}

func (s *Store) UpdateRequestReplyProbe(ctx context.Context, clusterID, probeID string, in domain.RequestReplyProbeUpdate) (domain.RequestReplyProbe, error) {
	current, err := s.GetRequestReplyProbe(ctx, clusterID, probeID)
	if err != nil {
		return domain.RequestReplyProbe{}, err
	}
	...
	_, err = s.pool.Exec(ctx, queryUpdateRequestReplyProbe,
		clusterID, probeID, subject, payload, format.String(), timeoutMs, enabled,
	)
	if err != nil {
		return domain.RequestReplyProbe{}, err
	}
	return s.GetRequestReplyProbe(ctx, clusterID, probeID)
}

func (s *Store) DeleteRequestReplyProbe(ctx context.Context, clusterID, probeID string) error {
	tag, err := s.pool.Exec(ctx, queryDeleteRequestReplyProbe, clusterID, probeID)
	if err != nil {
		return err
	}

...
func (s *Store) UpsertArchitectureScoreDaily(ctx context.Context, row domain.ArchitectureScoreDailyRow) error {
	...
	_, err = s.pool.Exec(ctx, queryUpsertArchitectureScoreDaily,
		row.ClusterID,
		day,
		row.Score,
...
func (s *Store) GetArchitectureScoreDaily(ctx context.Context, clusterID string, day time.Time) (domain.ArchitectureScoreDailyRow, bool, error) {
	var row domain.ArchitectureScoreDailyRow
	var factorsRaw []byte
	err := s.pool.QueryRow(ctx, queryGetArchitectureScoreDaily, clusterID, day.UTC()).Scan(
		&row.ClusterID,
		&row.ScoreDay,
		&row.Score,
		&factorsRaw,
...
func (s *Store) ListArchitectureScoreDaily(
	ctx context.Context,
	clusterID string,
	from, to time.Time,
) ([]domain.ArchitectureScoreDailyRow, error) {
	rows, err := s.pool.Query(ctx, queryListArchitectureScoreDaily, clusterID, from.UTC(), to.UTC())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) DeleteArchitectureScoreDailyOlderThan(ctx context.Context, cutoff time.Time) (int64, error) {
	tag, err := s.pool.Exec(ctx, queryDeleteArchitectureScoreDailyOlderThan, cutoff.UTC())
	if err != nil {
		return 0, err
	}

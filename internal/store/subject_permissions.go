...
func (s *Store) ListSubjectPermissions(ctx context.Context, clusterID, accountName, subject string) (SubjectPermissionsResult, error) {
	subject = strings.TrimSpace(subject)
	result := SubjectPermissionsResult{Subject: subject}

	users, err := s.ListNATSAccountUsers(ctx, clusterID, accountName)
	if err != nil {
		return result, err
	}
	groups, err := s.ListSigningGroups(ctx, clusterID, accountName)
	if err != nil {
		return result, err
	}

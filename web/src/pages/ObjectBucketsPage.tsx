...
export default function ObjectBucketsPage() {
  ...
  const [buckets, setBuckets] = useState<ObjectBucketInfo[]>([]);
  ...
  async function load() {
    if (!clusterId) return;
    try {
      const data = await api<BucketListResponse>(clusterPath(clusterId, "/objects/buckets"));
      setBuckets(data.buckets);
      setError("");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load object buckets");
    }
  }
        ...
        <table>
          ...
          <tbody>
            {buckets.map((b) => (
              <tr key={b.bucket}>
                <td>
                  <Link to={`/objects/${b.bucket}`}>{b.bucket}</Link>
                </td>
                <td>{b.description || "-"}</td>
                <td>{b.size}</td>
                <td>
